 ...
 * @private
 */
function _processFile ( _file, callback ) {

    try {

        var mtlLoader = new MTLLoader()
        var materials = mtlLoader.parse( _file )
...
function parseFile ( file, callback ) {
    ...
    if( ! file ) { throw new Error("Unable to parse empty, null or undefined mtl file !") }

    if( _DEBUG ) {
        console.log( "-------------------------------------------------------------------" )
        console.log( "Start parsing mtl file:" )
        console.log( file )
        console.log( "\n" )
    }
    ...
    readStream.on('readable', () => {
        ...
        if( _DEBUG ) console.log('mtlfile ' + file + ' readStream readable', readStream.read())
    ...
    readStream.on('data', (chunk) => {
        ...
        if( _DEBUG ) console.log('mtlfile ' + file + ' readStream received ' + chunkLength + ' bytes of data.')
    ...
    readStream.on('end', () => {
        ...
        if( _DEBUG ) console.log('mtlfile ' + file + ' readStream end')

    })

    readStream.on('close', () => {

        if( _DEBUG ) console.log('mtlfile ' + file + ' readStream close')
    ...
    _lineReader.on( 'line', function( line ) {
        ...
        if( _DEBUG ) console.log( 'mtlfile ' + file + ' Readline line:' + line )
    ...
    _lineReader.on( 'pause', function() {

        if( _DEBUG ) console.log('mtlfile ' + file + ' Readline paused.')

    } )

    _lineReader.on( 'resume', function( line ) {

        if( _DEBUG ) console.log('mtlfile ' + file + ' Readline resumed.')

    } )

    _lineReader.on( 'SIGCONT', function( line ) {

        if( _DEBUG ) console.log('mtlfile ' + file + ' Readline SIGCONT.')

    } )

    _lineReader.on( 'SIGINT', function( line ) {

        if( _DEBUG ) console.log('mtlfile ' + file + ' Readline SIGINT.')

    } )

    _lineReader.on( 'SIGTSTP', function( line ) {

        if( _DEBUG ) console.log('mtlfile ' + file + ' Readline SIGTSTP.');

    } )

    _lineReader.on( 'close', function () {

        if( _DEBUG ) console.log( 'mtlfile ' + file + ' Readline close' )

        _processFile( _file, _callback )
    ...
    function deleteTemporaryFile() {
        ...
        if( _DEBUG ) console.log( 'deleteTemporaryFile: ' + file )
